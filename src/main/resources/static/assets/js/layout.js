async function loadComponent(element, file) {
  ...
  document.querySelector(element).innerHTML = await response.text();
}
...
async function setupLayout() {
  ...
  if (roleEl) {
    roleEl.innerText = role.replace("_", " ");
  }

  // Highlight active link
  const path = window.location.pathname;
  document.querySelectorAll(".menu").forEach(link => {
    if (link.href && link.href.includes(path)) link.classList.add("active");
  });
}
...
const IC = {
  ...
  departments: `<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.8" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M3.75 21h16.5M4.5 3h15M5.25 3v18m13.5-18v18M9 6.75h1.5m-1.5 3h1.5m-1.5 3h1.5m3-6H15m-1.5 3H15m-1.5 3H15M9 21v-3.375c0-.621.504-1.125 1.125-1.125h3.75c.621 0 1.125.504 1.125 1.125V21"/></svg>`,
  pending:     `<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.8" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M12 6v6h4.5m4.5 0a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>`,
...
function buildMenu() {
  ...
  if (role === "HOST") {
    html += `<span class="sidebar-section-label">Host</span>
      <a class="menu" href="/pages/host/pending.html">${IC.pending} Pending Visitors</a>
      <a class="menu" href="/pages/host/visitors.html">${IC.visitors} My Visitors</a>`;
  }
...
function showToast(message, type = "success") {
  ...
  const successIcon = `<svg class="toast-icon" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M9 12.75L11.25 15 15 9.75M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>`;
  const errorIcon   = `<svg class="toast-icon" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M12 9v3.75m9-.75a9 9 0 11-18 0 9 9 0 0118 0zm-9 3.75h.008v.008H12v-.008z"/></svg>`;
